...
function [selRegion,numbReg, BW] = featureDetectionSb(img, w, pth, radius, show_figures)
...
BW = zeros(siz);
for i = 1:numbReg
%     selRegion(i,1).WeightedCentroid(1,1) = regionProperties(selected(i),1).Centroid(1,1);
%     selRegion(i,1).WeightedCentroid(1,2) = regionProperties(selected(i),1).Centroid(1,2);
%     selRegion(i,1).area = regionProperties(selected(i),1).area;
%     selRegion(i,1).Area = sum(sum((regionProperties(selected(i),1).area) ~= 0));
